...
class AuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(
        ...
        if not request.session.get("authenticated") or not request.session.get("store_id"):
            ...
            next_url = request.scope.get("path", request.url.path) or "/"
            if root_path:
                next_url = f"{root_path}{next_url}" if next_url != "/" else f"{root_path}/"
...
class RootPathRedirectMiddleware(BaseHTTPMiddleware):
    async def dispatch(
        ...
        if not root_path or not location.startswith("/"):
            return response

        if location == "/":
            response.headers["location"] = f"{root_path}/"
            return response

        if location == root_path or location.startswith(f"{root_path}/"):
            return response
