      ...
      <s-section heading="Acquisition and store health">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 18 }}>
          ...
          <div><h3 style={{ marginTop: 0 }}>Catalogue and content</h3><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[['Products', catalogue.products], ['Active', catalogue.active_products], ['SEO descriptions available', catalogue.seo_descriptions_available], ['Custom SEO descriptions', catalogue.custom_seo_descriptions], ['Automatic SEO fallbacks', catalogue.automatic_seo_descriptions], ['Missing SEO descriptions', catalogue.missing_seo_descriptions], ['Invalid product types', catalogue.invalid_product_types], ['Active with zero stock', catalogue.active_zero_stock], ['Blog articles', summary.shopify?.content?.articles]].map(([label, value]) => <div key={String(label)} style={{ padding: 12, background: "#f9fafb", borderRadius: 10 }}><strong style={{ display: "block", fontSize: "1.2rem" }}>{Number(value || 0).toLocaleString("en-GB")}</strong><span style={{ color: "#6b7280", fontSize: ".75rem" }}>{label}</span></div>)}
          </div></div>
        </div>
      </s-section>
